...
# - We intentionally do NOT use the word "true" anywhere (no "w_true", etc.)
#
...
def perceptron_forward(X, w, b, activation="step"):
    ...
    Returns:
        ...
        score: either z (step), p (sigmoid), or r (relu) depending on activation
    """
    z = X @ w + b
...
def draw_bio_neuron(ax, w1, w2, b, activation_name):
    ...
    # Weights labels
    ax.text((x_in1+x_sum)/2 - 0.02, (y_in1+y_sum)/2 + 0.02,
            f"w₁={w1:.2f}", fontsize=9)
    ax.text((x_in2+x_sum)/2 - 0.02, (y_in2+y_sum)/2 - 0.04,
            f"w₂={w2:.2f}", fontsize=9)
...
def demo(activation="step", dataset="linear", w1=1.0, w2=-1.0, b=0.0,
    ...
    plot_decision_boundary(
        ax1, X, y, w1, w2, b, activation, dataset_name,
